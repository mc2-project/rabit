 ...
 */
#define __ENCLAVE__
#ifndef RABIT_TLS_CONN_H_
#define RABIT_TLS_CONN_H_
...
class TLSConnection {
  public:
    ...
    inline int GetSockError(void) const {
      ...
      if (ctx.fd != INVALID_SOCKET && getsockopt(ctx.fd,  SOL_SOCKET, SO_ERROR,
    ...
    inline int AtMark(void) const {
      int atmark;
      if (ctx.fd != INVALID_SOCKET && ioctl(ctx.fd, SIOCATMARK, &atmark) == -1) return -1;
      return atmark;
    }
...
class TLSClient : public TLSConnection {
 public:
  ...
  inline bool Connect(const SockAddr &addr) {
    // Connect
    if ((ret = mbedtls_net_connect(&ctx, addr.AddrStr().c_str(),
  ...
  public:
    ...
    inline int TryBindHost(int start_port, int end_port) {
      ...
      for (int port = start_port; port < end_port; ++port) {
        if (mbedtls_net_bind(&ctx, NULL, std::to_string(port).c_str(), MBEDTLS_NET_PROTO_TCP) == 0) {
          return port;
        }
        if (errno != EADDRINUSE) {
          Error("TryBindHost");
    ...
    //void Accept(mbedtls_net_context *t_fd, mbedtls_ssl_context *t_ssl) {
    void Accept(TLSClient* client) {
      mbedtls_ssl_conf_rng( &conf, mbedtls_ctr_drbg_random, &ctr_drbg );

      if( ( ret = mbedtls_net_accept( &ctx, &client_fd, NULL, 0, NULL ) ) != 0 ) {
        print_err(ret);
      ...
      memcpy(&client->ctx, &client_fd, sizeof(mbedtls_net_context));
      memcpy(&client->ssl, &ssl, sizeof(mbedtls_ssl_context));
    }
};
